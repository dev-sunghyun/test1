...
function applyOption(optionContainer) {
	...
	let stockZip = optionContainer.querySelectorAll('.stockZip input');

	// 기존 processOption의 내용을 복사
	let existingOptions = Array.from(processOption.children).filter(child => child.tagName === "INPUT");

	let stockOption = document.querySelectorAll(`.optionpage-lego-option`);

	optionContainer.querySelector(".stockZip").innerHTML = "";
	optionContainer.querySelector(".processOption").innerHTML = "";

	let selectOp = document.querySelectorAll('.optionpage-lego-option');
	for (let i = 1; i <= parentDivLength; i++) {
		try {
			let selectOption = document.querySelector(`.optionpage-lego-option-select${i}`).value;
			console.log("selectOption : " + selectOption)
			
			console.log("selectOp : " + selectOp[i-1].value);
			let stockOp = stockOption[i - 1].value;

			let newInput = document.createElement("input");
			newInput.type = "text";
			newInput.value = selectOption;
			newInput.className = `option-value${i}`;
			newInput.setAttribute("name", `select${selectOp[i - 1].value + selectNum}`);
			...
			newInput2.type = "text";
			newInput2.value = stockOp;
			newInput2.readOnly = true;
			optionContainer.querySelector('.stockZip').appendChild(newInput2);
		} catch (error) {
			console.error(error);
			continue;
		}
